...
return new class extends Migration
{
    ...
    private string $name = 'biz_enterprise_bind';
    ...
    public function up(): void
    {
        ...
        && Schema::create($this->name, function (Blueprint $table) {
            ...
            $table->unique(['enterprise_id', 'module', 'mer_id'])->nullsNotDistinct();

            // ✅ 4. 外键约束（复用已存在的单列索引，零额外开销）
            $table->foreignId('enterprise_id')
                ->constrained('biz_enterprise')
                // ->cascadeOnDelete() // 主表记录删除时同步关联删除
                ->restrictOnDelete(); // 有关联时禁止删除主表记录
        });
